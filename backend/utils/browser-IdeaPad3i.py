...
class BrowserManager:
    ...
    def __init__(self, headless: bool = False, user_data_dir: str = "browser_session"):
        ...
        # Si la app está compilada (PyInstaller), guardar la sesión del navegador
        # en AppData para tener permisos de escritura (Program Files es de solo lectura).
        if getattr(sys, 'frozen', False) and not os.path.isabs(user_data_dir):
            appdata = os.environ.get('APPDATA', os.path.expanduser('~'))
            self.user_data_dir = os.path.join(appdata, "MonitoringKPIsCorporativos", user_data_dir)
        else:
