...
class ProyeccionOtsStHandler:
    ...
    async def ejecutar(self, lista_grupos: List[str], layout: str = "/BDytd_25_ot", suffix: str = "_ST"):
        ...
        ruta_final = os.path.join(os.environ.get("_MONITORING_OUTPUT_DIR", os.path.join(os.getcwd(), "output")), nombre_final)
    ...
    async def exportar_st(self) -> Optional[str]:
        ...
        try:
            ...
            temp_path = os.path.join(os.environ.get("_MONITORING_OUTPUT_DIR", os.path.join(os.getcwd(), "output")), f"tmp_ots_st_{datetime.now().strftime('%H%M%S')}.xlsx")
