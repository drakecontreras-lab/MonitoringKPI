    ...
    async def ejecutar(self, params: dict):
        ...
        try:
            ...
            async def get_otp_code() -> str:
                ...
                # threading.Event no es awaitable; usar run_in_executor
                loop = asyncio.get_event_loop()
                await loop.run_in_executor(None, self.app_api.mfa_event_proy.wait)
            ...
            if modo == "full_proyecciones":
                ...
                if activar_uts and lista_uts:
                    ...
                    if any(s in selected for s in ("trabajo_planificado", "programa_semanal", "plan_matriz")):
                        step += 1
                        self.actualizar_progreso(0.10 + (step/max(total_steps,1))*0.30)
                        await h_ordenes.ejecutar(lista_uts=lista_uts, layout="/KPIAT0610_M", suffix="", excel_plan_matriz=excel_plan_matriz)
                        await self.manejar_pausa()
                ...
                if activar_grupos and grupo_planif:
                    ...
                    if any(s in selected for s in ("trabajo_planificado", "programa_semanal", "plan_matriz")):
                        step += 1
                        self.actualizar_progreso(0.50 + (step/max(total_steps,1))*0.20)
                        await h_ordenes_diea.ejecutar(grupo_planif=grupo_planif, layout="/KPIAT0610_M", suffix="_DIEA")
                        await self.manejar_pausa()
                    if "ordenes" in selected:
                        step += 1
                        self.actualizar_progreso(0.50 + (step/max(total_steps,1))*0.20)
                ...
                await h_ordenes.ejecutar(lista_uts=lista_uts, layout="/KPIAT0610_M", suffix="",
